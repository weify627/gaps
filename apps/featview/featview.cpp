...
static RNInterval default_value_range(0.86,0.91);
...
UpdateQueryFeatures(void)
{
  ...
  // FW: assign mesh features to query features
  if (TRUE){
      int closest_idx = 0;
      for (int m = 0; m < meshes.NEntries(); m++) {
        R3Mesh *mesh = meshes.Kth(m);
        ...
        for (int i = 0; i < mesh->NVertices(); i++) {
          R3MeshVertex *vertex = mesh->Vertex(i);
          const R3Point& position = mesh->VertexPosition(vertex);
          RNScalar d = R3Distance(position, selected_position);
          if (d < closest_d) {
        ...
        //printf("  assign = %.2f %.2f %.2f seconds\n", position.X(), position.Y(), position.Z());
      }

      //printf("  assign = %d seconds\n", closest_idx);
      //printf("  assign = %d \n", point_features.NEntries());
      //printf("  assign = %.2f %.2f %.2f seconds\n", selected_position.X(), selected_position.Y(), selected_position.Z());
      RNDenseMatrix *features = point_features.Kth(0);
      query_features.Reset(features->NColumns());
      for (int j = 0; j < features->NColumns(); j++) {
        query_features[j] = (*features)[closest_idx][j];
      }
      //printf("  max0 = %.2f seconds\n", (*features)[closest_idx][0]);
      return;
  }
  // Generate features from scratch
  //printf("  Encode = %.2f seconds\n", 3.1);
